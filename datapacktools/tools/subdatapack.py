...
class SubDatapack:
    ...
    def choose_subfolder(self):
        ...
        if len(subfolders) == 1:
            subfolder_tk = tk.StringVar(value=subfolders[0])
            ...
            for index, folder in enumerate(subfolders):
                tk.Radiobutton(subfolder_frame, text=folder, variable=subfolder_tk, value=folder, tristatevalue=0).grid(row=index//2, column=index%2, sticky=tk.W)
        self.subfolder_tk = subfolder_tk
